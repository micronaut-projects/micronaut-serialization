...
from micronaut.context.annotation import Secondary
...
class ReversePointSerde(Serde[Point]):
    def deserialize(self, decoder: Decoder, context: DecoderContext, type: Argument) -> Point:
        array = decoder.decodeArray()
        y = array.decodeInt()  # <2>
        x = array.decodeInt()
        array.finishStructure()
        return Point.value_of(x, y)

    def serialize(self, encoder: Encoder, context: EncoderContext, type: Argument, value: Point) -> None:
        if value is None:
            raise ValueError("Point cannot be null")
        coords = value.coords()
        array = encoder.encodeArray(type)
