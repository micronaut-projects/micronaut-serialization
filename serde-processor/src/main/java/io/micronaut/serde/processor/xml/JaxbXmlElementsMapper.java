...
/**
 * Maps JAXB's {@code @XmlElements} choice declarations to unwrapped subtype metadata.
 *
 * @since 3.2
 */
public final class JaxbXmlElementsMapper implements NamedAnnotationMapper {
    static final String JAXB_XML_ELEMENT_DEFAULT = "jakarta.xml.bind.annotation.XmlElement$DEFAULT";

    @Override
    public String getName() {
        return "jakarta.xml.bind.annotation.XmlElements";
    }
    ...
    public List<AnnotationValue<?>> map(AnnotationValue<Annotation> annotation, VisitorContext visitorContext) {
        ...
        for (AnnotationValue<Annotation> element : annotation.getAnnotations(AnnotationMetadata.VALUE_MEMBER)) {
            ...
            if (type == null || JAXB_XML_ELEMENT_DEFAULT.equals(type.getName())) {
                continue;
            }
            String name = element.stringValue("name").orElse(null);
            ...
            if (name != null && !JaxbXmlRootElementMapper.DEFAULT.equals(name)) {
                subtype.member("names", new String[] {name});
            }
