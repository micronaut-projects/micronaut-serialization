...
/**
 * Maps JAXB's {@code @XmlElementRefs} choice declarations to unwrapped subtype metadata.
 *
 * @since 3.2
 */
public final class JaxbXmlElementRefsMapper implements NamedAnnotationMapper {
    private static final String JAXB_ELEMENT = "jakarta.xml.bind.JAXBElement";

    @Override
    public String getName() {
        return "jakarta.xml.bind.annotation.XmlElementRefs";
    }
    ...
    public List<AnnotationValue<?>> map(AnnotationValue<Annotation> annotation, VisitorContext visitorContext) {
        ...
        for (AnnotationValue<Annotation> element : annotation.getAnnotations(AnnotationMetadata.VALUE_MEMBER)) {
            ...
            element.stringValue("name").filter(name -> !JaxbXmlRootElementMapper.DEFAULT.equals(name))
