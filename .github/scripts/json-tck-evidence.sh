...

{
  ...
  echo "- JSON-P TCK: $(awk -F\" '/^jakarta-json-tck = "/ {print $2; exit}' gradle/libs.versions.toml)"
  echo "- JSON-B TCK: $(awk -F\" '/^jakarta-json-bind-tck = "/ {print $2; exit}' gradle/libs.versions.toml)"
