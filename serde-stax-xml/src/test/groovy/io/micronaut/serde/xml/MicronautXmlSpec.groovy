...
trait MicronautXmlSpec implements XmlSpec {
    ...
    def <T> T readXml(String xml, Class<T> tClass){
        xmlMapper.readValue(xml, tClass)
    }

    /**
     * Boots a dedicated {@link ApplicationContext} with the supplied Micronaut
     * configuration properties, resolves a configured {@link XmlObjectMapper}
     * from it, runs the read, then closes the context.
     */
    @Override
