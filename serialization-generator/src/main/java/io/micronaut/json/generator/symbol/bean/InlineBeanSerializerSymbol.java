...
public class InlineBeanSerializerSymbol implements SerializerSymbol {
    ...
    SerializableBeanDefinition introspect(ProblemReporter problemReporter, GeneratorType type, boolean forSerialization) {
        return SerializableBeanIntrospector.introspect(problemReporter, linker.typeResolutionContext, type.getClassElement(), type.getClassLevelAnnotations(), forSerialization);
    }
    ...
    private void visitDependencies(DependencyVisitor visitor, ProblemReporter problemReporter, GeneratorType type, boolean ser) {
        SerializableBeanDefinition definition = introspect(problemReporter, type, ser);
    ...
    private CodeBlock serializeImpl(
        ...
        SerializableBeanDefinition definition = introspect(generatorContext.getProblemReporter(), type, true);
    ...
    ConditionExpression<String> checkJsonViewEnabled(SerializableBeanDefinition.Property prop) {
    ...
    private void serializeSubTyping(
            ...
            CodeBlock readExpression,
            SerializableBeanDefinition.Subtyping subtyping,
    ...
    private CodeBlock getPropertyAccessExpression(CodeBlock beanReadExpression, SerializableBeanDefinition.Property prop) {
        if (prop.getter != null) {
