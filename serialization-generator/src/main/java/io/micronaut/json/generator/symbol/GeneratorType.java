...
import io.micronaut.json.WildcardArgument;
...
public class GeneratorType {
    ...
    private static CodeBlock toRuntimeFactory(ClassElement type, Function<String, CodeBlock> variableResolve) {
        if (type.isArray()) {
            final TypeName typeName = PoetUtil.toTypeName(type.getRawClassElement());
            CodeBlock block = CodeBlock.of("$T.class", typeName);
            return CodeBlock.of("$T.of($L)",
                    Argument.class,
                    block
            );
        } else if (type.isWildcard()) {
            final WildcardElement wildcardElement = (WildcardElement) type;
            final List<? extends ClassElement> upperBounds = wildcardElement.getUpperBounds();
            final List<? extends ClassElement> lowerBounds = wildcardElement.getLowerBounds();
            if (!lowerBounds.isEmpty()) {
                return CodeBlock.of("$T.ofLowerBounds(new $T[] {$L}, new $T[] {$L})",
                                    WildcardArgument.class,
                                    toRuntimeFactoryVarargs(lowerBounds, false, variableResolve));
            } else {
                return CodeBlock.of("$T.ofUpperBounds(new $T[] {$L}, new $T[] {$L})",
                                    WildcardArgument.class,
                                    toRuntimeFactoryVarargs(upperBounds, false, variableResolve));
            }
        } else if (type.isGenericPlaceholder()) {
            final GenericPlaceholderElement placeholderElement = (GenericPlaceholderElement) type;
            return variableResolve.apply(placeholderElement.getVariableName());
        } else {
            CodeBlock block = CodeBlock.of("$T.class", PoetUtil.toTypeName(type.getRawClassElement()));
            final List<? extends ClassElement> boundGenericTypes = type.getBoundGenericTypes();
            if (!boundGenericTypes.isEmpty()) {
                block = CodeBlock.of("$T.of($L, $L)",
                        Argument.class,
                        block,
                        toRuntimeFactoryVarargs(type.getBoundGenericTypes(), false, variableResolve));
            } else {
                block = CodeBlock.of("$T.of($L)", Argument.class, block);
            }
            return block;
        }
    }
