...
@Internal
public class SerializableTypeElementVisitor extends AbstractGeneratorVisitor<Object> implements TypeElementVisitor<Object, Object> {
    @Override
    ...
    public void visitClass(ClassElement element, VisitorContext context) {
        ...
        final List<AnnotationValue<SerializationMixin>> mixins = element.getAnnotationValuesByType(
                SerializationMixin.class);
        for (AnnotationValue<SerializationMixin> mixin : mixins) {
            visitMixin(element, context, mixin);
        }
    }
    ...
    private void visitMixin(
            ClassElement declaring,
            VisitorContext context,
            AnnotationValue<SerializationMixin> annotation) {
        ...
        if (!forClassResolved.isPresent()) {
            context.fail("The class declared by @SerializationMixin was not found on the classpath", declaring);
            return;
        }
