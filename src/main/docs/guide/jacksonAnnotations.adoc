...
A `@JsonCreator` constructor or factory method parameter is matched with the bean property that has the same Java name, and `@JsonIgnore` on the field, getter or setter of that property applies to the parameter the same way as in Jackson Databind:

- If the parameter or any accessor of the property is explicitly annotated with `@JsonProperty`, only the ignored accessor is dropped and the creator parameter is still bound.
- Otherwise the whole property is ignored and the creator parameter receives `null` (or its default value).
- A record component annotated with `@JsonIgnore` is always ignored, even if it is also annotated with `@JsonProperty`.

`@JsonIgnoreProperties` and access restrictions such as `@JsonProperty(access = READ_ONLY)` are applied to creator parameters as before.

A property with the same Java name but a different explicit `@JsonProperty` name is a different JSON property and its annotations are not applied to the creator parameter.
...
Note that when using these annotations it is recommended that you make `jackson-databind` a `compileOnly` (Maven `provided`) dependency since it is not needed at runtime:

dependency:jackson-databind[groupId="com.fasterxml.jackson.core",scope="compileOnly"]
