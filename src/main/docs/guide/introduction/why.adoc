...
In this run, generated Micronaut Serialization had the highest serialization throughput, about 19.4% faster than Jackson Databind and about 12.9% faster than the runtime fallback. It was also the fastest deserialization path, about 20.7% faster than Jackson Databind, and the fastest combined serialize-and-deserialize round trip, about 27.5% faster than Jackson Databind. The runtime fallback remained slower than generated Micronaut Serialization across all measured operations, which is expected because it uses runtime serializer/deserializer selection instead of generated classes.
