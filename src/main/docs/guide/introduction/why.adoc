...
In this run, generated Micronaut Serialization had the highest serialization throughput, about 34.1% faster than Jackson Databind and about 20.9% faster than the runtime fallback. It was also the fastest deserialization path, about 12.2% faster than Jackson Databind, and the fastest combined serialize-and-deserialize round trip, about 35.5% faster than Jackson Databind. The runtime fallback remained slower than generated Micronaut Serialization across all measured operations, which is expected because it uses runtime serializer/deserializer selection instead of generated classes.
