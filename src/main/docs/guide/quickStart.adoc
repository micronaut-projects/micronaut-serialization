...
NOTE: For Kotlin, add the `micronaut-serde-processor` dependency in https://docs.micronaut.io/4.4.3/guide/#kaptOrKsp[kapt or ksp scope], for Groovy add `micronaut-serde-processor` in compileOnly scope, and for Python add it to the `build` dependencies of `pyproject.toml` as shown above.
