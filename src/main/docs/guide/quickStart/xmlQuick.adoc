...
Names, transient properties, ordering, enum lexical values, and `type` overrides use shared serde metadata and can therefore affect other format implementations. JAXB defaults and nil policies are XML-only. Explicit Micronaut or Jackson configuration wins when it conflicts with JAXB metadata. Unsupported JAXB features include `@XmlAccessorType`, package `@XmlSchema`, choices/refs, adapters, `JAXBElement`, schema generation, and `JAXBContext`; unsupported annotation members include all `required` flags, `@XmlType.name`, `@XmlType.namespace`, `@XmlType.factoryClass`, `@XmlType.factoryMethod`, and `@XmlEnum.value`.
