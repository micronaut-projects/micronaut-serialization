...
`maximum-input-size` defaults to `10MB` and stops XML parsing after that many input bytes. Increase it only when trusted payloads legitimately exceed the default; transport-level request limits should remain enabled as the first line of defense.
